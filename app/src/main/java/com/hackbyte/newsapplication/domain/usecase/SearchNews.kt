...
class SearchNews @Inject constructor(private val newsApiRepository: NewsApiRepository) {

    suspend operator fun invoke(query: String = ""): Flow<Response<NewsApiModel>> {
        return flow {
            try {
                emit(Response.Loading())
                val news = newsApiRepository.searchNews(query)
